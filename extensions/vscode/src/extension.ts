...
function copyInterfaceFile(folder : vscode.Uri, interfacePath : string) {
    let extensionfolder = vscode.extensions.getExtension("tracktwo.unrealscript-debugger").extensionPath;
...
function installInterface() {
    vscode.window.showQuickPick(
        [
          { "label": "Install 32-bit debugger interface",  "interfacePath": "bin/win32/" },
          { "label": "Install 64-bit debugger interface", "interfacePath": "bin/win64/" }
        ],
        { canPickMany: false }
    ).then( (pick) => {
            if (pick)
